...
class Tables(sqlContext: SQLContext, scaleFactor: Int) extends Serializable {
  ...
  case class Table(name: String, partitionColumns: Seq[String], fields: StructField*) {
    ...
    val partitions = if (partitionColumns.isEmpty) 20 else 20
    ...
    def genData(
      ...
      log.info(s"Begin genData for table $name in database to $location")
      val mode = if (overwrite) SaveMode.Overwrite else SaveMode.Ignore

      val data = df(format != "text")
      log.info(s"Got dataframe for format $format")
      val tempTableName = s"${name}_text"
      data.registerTempTable(tempTableName)
      log.info(s"Registered temp table $tempTableName")
      ...
      val writer = if (partitionColumns.nonEmpty) {
        if (clusterByPartitionColumns) {
          ...
          val partitionColumn = s"_${partitionColumns.head}";
          val duplicated = data.withColumn(partitionColumn, $"${partitionColumns.head}")
          duplicated.write.partitionBy(partitionColumn)
        }
      } else {
      ...
      writer.format(format).option("delimiter", "|").mode(mode)
